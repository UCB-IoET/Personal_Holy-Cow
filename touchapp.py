...
class pathApp(driver.SmapDriver):
	...
	if self.assist == false:
		...
		print "Touch path src callback"
		print "Points:", points
		print "\nData:", path_src
		self.touch_index = path_src[0][0][-1] +0.0
	...
	print self.table
		...
		self.path = self.getLedIndices(int(src), int(dst))
		
	if self.k < self.path.amount():
		index = self.path[k]
		self.k = self.k+1	
		print(index)
		if(self.touch_index != index)
			return
		curr = time.time()
               	self.add('/rgb', curr, 31 + 0.0)
		self.add('/index', curr, index + 0.0)
		
		index = self.path[self.k]
		time.sleep(1)
               	self.add('/rgb', curr, 0 + 0.0)
		self.add('/index', curr, index-1 + 0.0)
		time.sleep(1)
	if self.k >= self.path.amount()
		self.assist = false
		self.k=0	
